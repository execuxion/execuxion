...
class WorkflowEngine {
  ...
  async destroy(status, message, blockDetail) {
    ...
    try {
      ...
      // COMMENTED OUT: Backend log reporting - logs are now stored locally in IndexedDB
      // Logs are already saved via this.logger.add() below (line 563)
      ...
      // Store workflow execution summary locally for analytics
      if (!this.workflow.settings?.debugMode) {
        ...
        try {
          // Get existing execution history
          const { executionHistory = [] } = (await BrowserAPIService.storage.local.get(
            'executionHistory'
          )) || {};

          // Keep only last 100 executions to prevent storage bloat
          const updatedHistory = [executionSummary, ...executionHistory].slice(0, 100);

          await BrowserAPIService.storage.local.set({
            executionHistory: updatedHistory
          });
        } catch (err) {
          console.error('Failed to store execution summary locally:', err);
        }
      }
