...
class WorkflowState {
  ...
  _updateBadge() {
    // Desktop app doesn't have browser badge, skip this
    // In browser extension, this would update the extension icon badge
    try {
      if (BrowserAPIService.action?.setBadgeText) {
        BrowserAPIService.action.setBadgeText({
          text: (this.states.size || '').toString(),
        });
      }
    } catch (error) {
      // Ignore badge update errors in desktop mode
    }
  }
