...
export async function loadLocaleMessages(locale, location) {
  ...
  const importLocale = async (path, merge = false) => {
    try {
      // For Vite, we need to fetch JSON files directly
      const response = await fetch(`/src/locales/${locale}/${path}`);
      ...
      console.error(`Failed to load locale file: /src/locales/${locale}/${path}`, error);
    }
  };
