<template>
  <div v-if="workflow" class="flex" style="height: calc(100vh - 40px - var(--titlebar-height))">
    <div
      ...
      >
        ...
        <ui-tab-panel cache value="editor" class="w-full" @keydown="onKeydown">
          ...
          <workflow-editor
            ...
            style="height: calc(100vh - 40px - var(--titlebar-height))"
            tabindex="0"
            @init="onEditorInit"
            @edit="initEditBlock"
...
async function executeFromBlock(blockId) {
  try {
    ...
    // Use WorkflowManager for desktop app execution
    const { default: WorkflowManager } = await import('@/workflowEngine/WorkflowManager');
    const workflowManager = WorkflowManager.instance;

    // Execute workflow from specific block
    workflowManager.execute(workflow.value, {
      blockId: blockId,  // FIX: Changed from options.startBlockId to direct blockId parameter
      data: {
        variables: {},
        globalData: {}
      }
    ...
    toast.success(`Workflow execution started from block ${blockId.substring(0, 7)}...`);
  } catch (error) {
    console.error('Failed to execute workflow from block:', error);
...
function onKeydown(event) {
  const { ctrlKey, metaKey, shiftKey, key, target, repeat } = event;
  if (repeat) return;
  ...
  if (command('c')) {
    ...
    // Select all blocks in the workflow editor
    if (editor.value) {
      const allNodes = editor.value.getNodes.value;
      editor.value.addSelectedNodes(allNodes);
    }
  }
}
